...
select str_to_date('2025/12/03', '%Y/%d/%m');
select date_format(now(), '%m/%d %H:%i:%s'); -- 이것과 관련된 문제가 가장 많이 나옴 (ex. 특정한 형태의 date를 표현하시오.)
...
-- ex.
select concat('A', ',', ifnull(null, ''), 'B', ':', 'C'), concat_ws(',', 'A', null, 'B', 'C'); -- 실무에서는 infull 패턴을 더 많이 씀
--
...
select ELT(FIELD('N', 'Y', 'Y'), 'Active', 'Inactive');
select ELT(1, 'Active', 'Inactive');
...
select insert('12/5', 3, 2, '/'); -- 12/5 -- (문자열, 위치, 제거수, 추가문자) splice(2, 2, '/')
...
select reverse('abc'), repeat('x', 9), cocnat('|', space(5), '|'),
	   replace('abcdefgcdeyy', 'cde', 'xxx');
select trim(leading 's' from 'ssstrss'), ltrim('ssstrss');

select substring('abcde', 2, 3); -- select substring('Bearer accessTokenasdafdsafdsff', 7);
select substring_index('a,b,c', ',', 2);  -- a,b (,를 기준으로 두번째까지)
select substring_index('a,b,c', ',', -2); -- c  (-1은 ,로 잘라낸 마지막 값) -- -2까지 (뒤에서 두번째까지)
...
select weekday('2025-12-25');   -- 월요일부터 시작 0 ~ 6(일요일)
select dayofweek('2025-12-25'); -- 일요일부터 시작 1 ~ 7(토요일)

select DATE(now()), TIME(now()), MAKEDATE(2020, 336), MAKETIME(19,3,50); -- make 함수 두개를 합치면 datetime이 된다. (with concat)
...
SELECT REGEXP_LIKE('abc', 'AB', '1'); -- c: case sensitive
...
select name, regexp_replace(name, '.{2}', '**', 2, 1) from Student; -- .은 모든 글자 의미

-- 전화번호가 '010-2323-4545', '010.2323.4545', '010 2323 4545' 등 다양한 형태로 되어있다. 숫자만 추출하시오.
select regexp_replace('010-2323-4545', '[^0-9]', ''); -- 대괄호 밖의 ^는 부터를, 대괄호 안의 ^는 not을 의미
...
SELECT 
  ...
  CASE
    ...
    ELSE 
      CONCAT(
        LEFT(name, FLOOR(CHAR_LENGTH(name) / 2)),
        '*',
        RIGHT(name, CEIL(CHAR_LENGTH(name) / 2) - 1)
      )
  END AS masked_name
FROM Student;

SELECT 
  '010-2323-4545' AS origin,
  REGEXP_REPLACE('010-2323-4545', '[^0-9]', '') AS only_number
UNION ALL
SELECT 
  '010.2323.4545',
  REGEXP_REPLACE('010.2323.4545', '[^0-9]', '')
UNION ALL
SELECT 
  '010 2323 4545',
  REGEXP_REPLACE('010 2323 4545', '[^0-9]', '');
